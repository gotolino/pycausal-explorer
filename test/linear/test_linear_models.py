...
def test_causal_logistic_regression_init_raise_exception():
    with pytest.raises(ValueError):
        CausalLogisticRegression(covariates="x0")
        CausalLogisticRegression(treatment=["t"])


def test_causal_linear_regression_init_raise_exception():
    with pytest.raises(ValueError):
        CausalLinearRegression(covariates="x0")
        CausalLinearRegression(treatment=["t"])
...
def test_causal_linear_regression_train():
    ...
    causal_linear_regression = CausalLinearRegression(covariates=["x0"], treatment="t")
