...
class CausalLinearRegression(BaseCausalModel):
    ...
    def fit(self, X, y):
        ...
        self.fitted_model = LinearRegression(n_jobs=-1).fit(X_std[[self.treatment] + self.covariates], y)
        self.fitted_model_params_ = [self.fitted_model.intercept_] + self.fitted_model.coef_.tolist()
...
class CausalLogisticRegression(BaseCausalModel):
    ...
    def predict_proba(self, X):
        X_std = X.copy()
        X_std[self.covariates] = self.standard_scaler.fit_transform(X[self.covariates])
        return self.fitted_model.predict_proba(X_std[[self.treatment] + self.covariates])[:, 1]
    ...
    def predict_ite(self, X):
        ...
        return (
            self.fitted_model.predict_proba(
                X_std_treated[[self.treatment] + self.covariates]
            )[:, 1] - self.fitted_model.predict_proba(
                X_std_control[[self.treatment] + self.covariates]
            )[:, 1]
        )
